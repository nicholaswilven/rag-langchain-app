...
class StellaEmbeddingModel:
    def __init__(self, model_name: str = "dunzhang/stella_en_400M_v5", vector_dim: int = 512, query_prompt = None):
    ...
    def download_model(self):        
        self.local_dir = os.path.join(r".\data\models", self.model_name.replace("/", "--"))
        ...
        for file in files:
            if "/" not in file:  # Exclude folders (no '/' in file path)
                if not os.path.exists(os.path.join(self.local_dir, file)):
                    downloaded_file = hf_hub_download(repo_id = self.model_name, filename = file, local_dir = self.local_dir)
    ...
    def load_model(self):
        ...
        vector_linear_dict = {
            ...
            torch.load(os.path.join(self.local_dir, rf"{vector_linear_directory}\pytorch_model.bin")).items()
        }
